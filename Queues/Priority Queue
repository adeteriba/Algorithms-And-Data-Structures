/*
Problem Statement
Manipulate a priority queue according to the given INSERT and REMOVE commands and then output the string that is at the front of the priority queue. If a remove command is issued for an empty queue then nothing should happen. Inserted strings should be entered into the priority queue according to the length of the string, with shorter strings at the front of the queue and longer strings at the back. Where two strings have equal length, positioning should be resolved by alphabetical ordering, with the words closer to the front of the dictionary (e.g. aardvark, apple) going towards the front of the priority queue, and the ones closer to the end of the dictionary going towards the back (e.g. zip, zoo). 
...
Input Format
An int n indicating the number of instructions, 
followed by n instructions each on a separate line (either INSERT followed by a string to insert, or REMOVE)
